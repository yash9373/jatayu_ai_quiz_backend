...
class TestRepository:
    ...
    async def get_test_by_id(self, test_id: int, created_by: int = None) -> Optional[Test]:
    ...
    async def get_tests_by_recruiter(self, recruiter_id: int, skip: int = 0, limit: int = 100) -> List[Test]:
        ...
        try:
            ...
            logger.error(f"Error getting tests for recruiter {recruiter_id}: {str(e)}")
            return []
    
    async def update_test(self, test_id: int, test_data: TestUpdate, updated_by: int, created_by: int = None) -> Optional[Test]:
    ...
    async def update_test_ai_data(self, test_id: int, parsed_jd: Dict[str, Any], skill_graph: Dict[str, Any]) -> Optional[Test]:
        ...
        try:
            ...
            await self.db.rollback()
            logger.error(f"Error updating AI data for test {test_id}: {str(e)}")
            raise
    ...
    async def update_test_schedule(self, test_id: int, schedule_data: Dict[str, Any]) -> Optional[Test]:
        ...
        try:
            ...
            await self.db.rollback()
            logger.error(f"Error updating schedule for test {test_id}: {str(e)}")
            raise
    ...
    async def update_is_published(self, test_id: int, is_published: bool) -> Optional[Test]:
        ...
        try:
            ...
            logger.info(f"Updated is_published for test {test_id} to {is_published}")
            return test
